...
public class ResponseSessionDao {
	...
	/** respondents を作成して respondent_id を返す（respondent_keyはUUIDでユニーク） */
	...
	/** response_sessions を作成して response_id を返す */
	public long createResponseSession(long surveyId, long respondentId) {
		jdbcTemplate.update("""
			INSERT INTO response_sessions (survey_id, respondent_id, status, started_at, created_at, updated_at)
	...
	public void assertBelongsToSurvey(long responseId, long surveyId) {
		Integer count = jdbcTemplate.queryForObject("""
			...
			WHERE response_id = ? AND survey_id = ?
